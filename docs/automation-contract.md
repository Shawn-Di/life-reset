...
The one automation stores one saved `Life-reset` conversation ID. During setup or repair, it lists or searches accessible conversations by the exact title `Life-reset`. It uses the saved ID when it is still accessible; otherwise it uses the most recently active accessible exact-title conversation and refreshes the saved ID. It creates a replacement only when no accessible exact-title conversation exists. Creating a replacement conversation is not permission to create a replacement automation. On Codex, the automation is a heartbeat attached to that conversation; its assistant response is the reminder, and it never uses `send_message_to_thread` for delivery. Because Codex displays the scheduled-task input, the saved heartbeat prompt is only `$life-reset`; durable instructions remain in the Skill.

The default schedule is exactly 08:00, 10:00, 12:00, 14:00, 16:00, 18:00, and 20:00 in the user's local timezone, inside the 08:00–22:00 daytime window. A custom window changes the same automation. It uses one fixed recurring schedule without minute jitter. If execution starts a few minutes after an exact hour, select that hour's slot and do not create an extra reminder.

On first activation without `displayName`, the assistant response is exactly “我该怎么称呼你？” and the conversation enters `awaiting-name`. That run ends immediately and pauses the heartbeat, preventing scheduled trigger messages while waiting. Only a new real user-authored message in the `Life-reset` conversation may supply the name; automation, assistant, tool, system, and cross-task messages are never answers, and the automation must never reply on the user's behalf. After the user answers, save the name and reactivate the same heartbeat. Later reminders are one line: `{displayName}，` followed by the selected question, a space, and the selected action. `Hi`, greeting-only lines, and line breaks are not used. Platform setup questions, status, source metadata, progress commentary, run records, delivery receipts, inbox items, and Skill initialization instructions must not be sent as reminder content.

Automation lookup, deduplication, conversation-ID persistence, and delivery success are silent. A successful run emits only the question or reminder; an awaiting-user run emits nothing. It must not emit “未读取到”, “已处理”, “已发送”, “已记录”, duplicate cleanup, an inbox item, or other execution status.
