...
On first activation without `displayName`, the assistant response is exactly “我该怎么称呼你？” and the conversation enters `awaiting-name`. That run ends immediately and pauses the heartbeat, preventing scheduled trigger messages while waiting. Only a new real user-authored message in the `Life-reset` conversation may supply the name; automation, assistant, tool, system, and cross-task messages are never answers, and the automation must never reply on the user's behalf. After the user answers, save the name and reactivate the same heartbeat. Later reminders are one line: `{displayName}，` followed by the selected question, a space, and the selected action, with no more than 40 Chinese characters after the name. `Hi`, greeting-only lines, and line breaks are not used. Platform setup questions, status, source metadata, progress commentary, run records, delivery receipts, inbox items, and Skill initialization instructions must not be sent as reminder content.
