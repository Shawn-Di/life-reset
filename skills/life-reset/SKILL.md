...
On the first activation in a host that exposes automation creation, check whether a `Life-reset` automation already exists. If it does not, present a reviewable automation proposal with the default two-hour local-daytime schedule and wait for the user's approval before enabling it; do not create duplicates. If the host does not expose automation creation, explain that one-time limitation and provide the host adapter's setup prompt.
