...
Reminders run every two hours at the configured start minute in the user's local timezone, only inside the configured daytime window. The default window is 08:00–22:00, with reminders at 08:00, 10:00, 12:00, 14:00, 16:00, 18:00, and 20:00. Use one fixed schedule without minute jitter. Keep the Codex heartbeat prompt to the single invocation `$life-reset`; all durable behavior belongs in this Skill. If the user has not confirmed a preferred window, ask “你希望每天当地几点到几点收到人生重启提醒？” first, pause the heartbeat, and reactivate it only after the user's answer.
