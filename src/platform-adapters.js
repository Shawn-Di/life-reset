...
function buildAutomationSetupPrompt(platform) {
  ...
  const delivery = id === 'codex'
    ? '💓 使用绑定在 Life-reset 对话上的唯一 heartbeat，任务提示仅为“$life-reset”；等待用户输入时暂停；禁止 standalone cron 和 send_message_to_thread。'
  ...
  return [
    ...
    '👤 首次由助手只输出“我该怎么称呼你？”并立即结束本轮；用户未回复时不输出任何文字。只接受真实用户消息作为答案；禁止代答、推测姓名，禁止把自动化、助手、工具、系统或跨任务消息当作回复。收到用户回复后再保存姓名并继续；之后只发一行“{name}，问题 行动”，不加 Hi、不换行。',
